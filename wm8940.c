...
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_INPUT_CTRL);
    return val & 0x0007;
}
...
void WM8940_Set_PGAInput(wm8940_input_t input)
{
    ...
    val &= ~(0x0007);
    val |= (uint16_t)input;
    WM8940_I2C_WRITE(WM8940_REG_INPUT_CTRL, val);
}

uint8_t WM8940_Get_PGAInput_Volume()
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_INPPGA_GAIN_CTRL);
    return val & 0x003F;
}

void WM8940_Set_PGAInput_Volume(uint8_t regval)
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_INPPGA_GAIN_CTRL);
    val &= ~(0x003F);
    val |= (regval & 0x003F);
    WM8940_I2C_WRITE(WM8940_REG_INPPGA_GAIN_CTRL, val);
}

// TODO
void WM8940_Set_PGAInput_Volume_db(float vol_db)
{
    if (vol_db < -12)
        vol_db = -12;
...
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_POWER_MANAGEMENT_2);
    val &= ~(1 << 2);
    val |= (state ? 1 : 0) << 2;
    WM8940_I2C_WRITE(WM8940_REG_POWER_MANAGEMENT_2, val);
}

void WM8940_Set_Aux_Enable(uint8_t state)
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_POWER_MANAGEMENT_1);
    val &= ~(1 << 6);
    val |= (state ? 1 : 0) << 6;
    WM8940_I2C_WRITE(WM8940_REG_POWER_MANAGEMENT_1, val);
}
...
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_ADC_BOOST_CTRL);
    val &= ~(1 << 8);
    val |= (state ? 1 : 0) << 8;
    WM8940_I2C_WRITE(WM8940_REG_ADC_BOOST_CTRL, val);
}
...
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_SPK_MIXER);
    val &= ~(0x03 << 0);
    val &= ~(1 << 5);
    if (source == WM8940_OUTPUT_FROM_AUX) source = (1 << 5);
    val |= source;
    WM8940_I2C_WRITE(WM8940_REG_SPK_MIXER, val);
}

void WM8940_Set_Speaker_FromBypass_Attenuation(uint8_t state)
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_SPK_VOL_CTRL);
    val &= ~(1 << 8);
    val |= (state ? 1 : 0) << 8;
    WM8940_I2C_WRITE(WM8940_REG_SPK_VOL_CTRL, val);
}
...
void WM8940_Set_Speaker_Volume_db(int8_t vol_db)
{
    ...
    WM8940_Set_Speaker_Volume(regval);
}

void WM8940_Set_Mono_Source(wm8940_speaker_source_t source)
{
    uint16_t val = WM8940_I2C_READ(WM8940_REG_MONO_MIXER_CTRL);
    val &= ~(0x07 << 0);
    val |= source;
    WM8940_I2C_WRITE(WM8940_REG_MONO_MIXER_CTRL, val);
}
...
void WM8940_Set_Output_Enable(wm8940_output_t output)
{
    ...
    WM8940_I2C_WRITE(WM8940_REG_POWER_MANAGEMENT_3, val);
}
